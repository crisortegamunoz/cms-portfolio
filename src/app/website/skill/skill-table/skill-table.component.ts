...
export class SkillTableComponent  extends UnsubscribeOnDestroyAdapter implements OnInit {
    ...
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
      if (result === 1) {
        ...
        this.exampleDatabase?.dataChange.value.unshift(
          this.skillService.getDialogData()
        );
        this.refreshTable();
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<SkillDTO>(true, []);
    });
    this.showNotification(
      'snackbar-danger',
  ...
  public loadData() {
    this.exampleDatabase = new SkillService(this.httpClient);
...
export class ExampleDataSource extends DataSource<SkillDTO> {
  ...
  filteredData: SkillDTO[] = [];
  renderedData: SkillDTO[] = [];
  constructor(
    public exampleDatabase: SkillService,
  ...
  sortData(data: SkillDTO[]): SkillDTO[] {
